...
class BookingController extends Controller
{
    ...
    {
        return \App\Models\BlockoutDate::where('start_date', '<', $checkOut)->where('end_date', '>', $checkIn)->exists();
    }
    ...
    private function getAvailableTents(string $checkIn, string $checkOut, int $guests, int $adults = 0, string $sort = 'recommended')
    {
        ...
        if ($sort === 'price_low') {
            $query
                ->addSelect([
                    'min_price' => Price::selectRaw('MIN(CASE WHEN tents.pricing_type = "person" THEN adult_price ELSE price_weekday END)')->whereColumn('tent_id', 'tents.id'),
                ])
    ...
    public function user(Request $request)
    {
        ...
        $tents = $this->getAvailableTents($currentDate, $nextDate, $guests, $adults, $sort)->paginate(4)->withQueryString();

        $upcomingBlockouts = \App\Models\BlockoutDate::where('end_date', '>=', today())->orderBy('start_date', 'asc')->get();
    ...
    public function myBookings(Request $request)
    {
        ...
        $query = Booking::with('slot.tent')->where('user_id', auth()->id());
        ...
        if ($request->filled('date')) {
            ...
            $query->where(function ($q) use ($date) {
                $q->where('check_in_date', '<=', $date)->where('check_out_date', '>=', $date);
            });
        }
            ...
            $query
                ...
                ->whereDoesntHave('bookings', function ($q2) use ($checkIn, $checkOut, $excludeBookingId) {
                    ...
                    $q2->where('check_in_date', '<', $checkOut)
                        ->where('check_out_date', '>', $checkIn)
                        ->where(function ($q3) {
                            $q3->where('status', 'confirmed')->orWhere(function ($q4) {
                                $q4->where('status', 'pending')->where('expires_at', '>', now());
                            });
                        });
                });
        })->get(['id', 'name']);
        ...
        if ($booking->slot->tent_id != $tentId || $booking->check_in_date != $checkIn || $booking->check_out_date != $checkOut) {
            ...
            $availableSlot = Slot::where('tent_id', $tentId)
                ...
                ->whereDoesntHave('bookings', function ($query) use ($checkIn, $checkOut, $booking) {
                    $query
                        ->where('id', '!=', $booking->id) // Ignore the current booking
                        ->where('check_in_date', '<', $checkOut)
                        ->where('check_out_date', '>', $checkIn)
                        ->where(function ($q3) {
                            $q3->where('status', 'confirmed')->orWhere(function ($q4) {
    ...
    public function adminOccupancy(Request $request)
    {
        ...
        $bookings = Booking::with('user', 'slot')
            ...
            ->where(function ($query) use ($startDate, $endDate) {
                $query->where('check_in_date', '<', Carbon::parse($endDate)->addDay())->where('check_out_date', '>', $startDate);
            })
            ->get();
