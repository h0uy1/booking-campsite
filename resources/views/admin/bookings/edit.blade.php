    ...
    <div class="bg-gray-50 min-h-screen py-12">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8 text-center">
                <h1 class="text-3xl font-extrabold text-gray-900 sm:text-4xl">Edit Booking #{{ str_pad($booking->id, 5, '0', STR_PAD_LEFT) }}</h1>
                <p class="mt-2 text-lg text-gray-500">Modify reservation dates, campsite, customer details, or pricing.</p>
            </div>
            ...
            <form action="{{ route('admin.bookings.update', $booking->id) }}" method="POST" class="space-y-8">
                ...
                <div class="bg-white shadow-xl rounded-2xl overflow-hidden border border-gray-100">
                    <div class="px-8 py-6 border-b border-gray-100 bg-gray-50/50">
                        ...
                        <p class="text-sm text-gray-500 mt-1">If you change dates, the available campsite list will update automatically.</p>
                    </div>
                    ...
                    <div class="p-8 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="date" name="check_in_date" id="check_in_date" value="{{ old('check_in_date', \Carbon\Carbon::parse($booking->check_in_date)->format('Y-m-d')) }}" required class="mt-2 block w-full shadow-sm sm:text-sm border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500 py-3 px-4">
                            </div>

                            <!-- Check-out -->
                            <div>
                                <label for="check_out_date" class="block text-sm font-bold text-gray-700">Check-out Date <span class="text-red-500">*</span></label>
                                <input type="date" name="check_out_date" id="check_out_date" value="{{ old('check_out_date', \Carbon\Carbon::parse($booking->check_out_date)->format('Y-m-d')) }}" required class="mt-2 block w-full shadow-sm sm:text-sm border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500 py-3 px-4">
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <select id="tent_id" name="tent_id" required disabled class="mt-2 block w-full shadow-sm sm:text-sm border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500 py-3 px-4 disabled:bg-gray-100 disabled:text-gray-500">
                                    <option value="" disabled selected>Loading availability...</option>
                                </select>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-white shadow-xl rounded-2xl overflow-hidden border border-gray-100">
                    <div class="px-8 py-6 border-b border-gray-100 bg-gray-50/50">
                        ...
                        <p class="text-sm text-gray-500 mt-1">Update the guest's contact info as needed.</p>
                    </div>
                    ...
                    <div class="p-8 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="customer_name" id="customer_name" value="{{ old('customer_name', $booking->customer_name ?? $booking->user->name ?? '') }}" required class="mt-2 block w-full shadow-sm sm:text-sm border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500 py-3 px-4">
                            </div>

                            <div>
                                <label for="customer_email" class="block text-sm font-bold text-gray-700">Email Address <span class="text-red-500">*</span></label>
                                <input type="email" name="customer_email" id="customer_email" value="{{ old('customer_email', $booking->customer_email ?? $booking->user->email ?? '') }}" required class="mt-2 block w-full shadow-sm sm:text-sm border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500 py-3 px-4">
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input type="text" name="customer_phone" id="customer_phone" value="{{ old('customer_phone', $booking->customer_phone) }}" class="mt-2 block w-full shadow-sm sm:text-sm border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500 py-3 px-4">
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input type="text" name="customer_address" id="customer_address" value="{{ old('customer_address', $booking->customer_address) }}" class="mt-2 block w-full shadow-sm sm:text-sm border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500 py-3 px-4">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white shadow-xl rounded-2xl overflow-hidden border border-gray-100">
                    <div class="px-8 py-6 border-b border-gray-100 bg-gray-50/50">
                        ...
                        <p class="text-sm text-gray-500 mt-1">Update the total pricing or change the status.</p>
                    </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <div class="relative mt-2 rounded-md shadow-sm">
                                    ...
                                    <input type="number" step="0.01" name="total_price" id="total_price" value="{{ old('total_price', $booking->total_price) }}" required class="block w-full pl-12 py-3 shadow-sm sm:text-lg border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500" placeholder="0.00">
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <select id="status" name="status" class="block w-full pl-3 pr-10 py-3 text-base border-gray-300 focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm rounded-lg border">
                                    <option value="confirmed" {{ old('status', $booking->status) == 'confirmed' ? 'selected' : '' }}>Confirmed (Paid)</option>
                                    <option value="pending" {{ old('status', $booking->status) == 'pending' ? 'selected' : '' }}>Pending Payment</option>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const originalOldTentId = "{{ old('tent_id', $booking->slot->tent_id) }}";
        ...
        function checkAvailability() {
            ...
            fetch(`/admin/bookings/tents?check_in_date=${checkIn}&check_out_date=${checkOut}&exclude_booking_id=${excludeBookingId}`)
